...
COPY ./docker/.theanorc .
COPY ./docker/keras.json .keras/
COPY ./docker/jupyter_notebook_config.py .jupyter/
...
RUN conda install -y --quiet -c conda-forge jupyter_contrib_nbextensions jupyter_nbextensions_configurator

RUN conda install -y --quiet -c plotly plotly
